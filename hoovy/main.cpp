  ...
  if (c->pix_fmt != AV_PIX_FMT_YUV420P)
  {
    ...
    if (!ost->sws_ctx)
    {
      ost->sws_ctx = sws_getContext(
          c->width, c->height, AV_PIX_FMT_YUV420P, c->width, c->height, c->pix_fmt, SCALE_FLAGS, NULL, NULL, NULL);
      if (!ost->sws_ctx)
      {
...
main(int argc, char **argv)
{
  ...
  while (encode_video || encode_audio)
  {
    ...
    if (encode_video &&
        (!encode_audio ||
         av_compare_ts(video_st.next_pts, video_st.enc->time_base, audio_st.next_pts, audio_st.enc->time_base) <= 0))
    {
